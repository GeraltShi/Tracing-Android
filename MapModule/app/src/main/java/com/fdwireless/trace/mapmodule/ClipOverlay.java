...
public class ClipOverlay {
 ...
 //private AMapLocation location;
    ...
    private my_StringRequest stringRequest=new my_StringRequest(context);
    public ArrayList<Clip> getClipList() {
        return ClipList;
    }
    private ClipOverlay(Context context,AMap amap,Player player,AppCompatActivity mMainActivity)
    {
    ...
    public static ClipOverlay getInstance(Context context,AMap amap,Player player,AppCompatActivity mMainActivity)
    {
        if(mClipOverlay == null)
            mClipOverlay = new ClipOverlay(context,amap,player,mMainActivity);
        return mClipOverlay;
    }
       ...
       // params.put("maxLon",maxLon+"");
        ...
        request.post(URL, Clip[].class, new my_CallBackListener<Clip[]>() {
            ...
            public void onErrorResponse(VolleyError error) {
                Toast.makeText(context,"载入标签信息错误：请检查网络连接",Toast.LENGTH_SHORT).show();
            }
        },params);
        ...
        while(itrTail < ClipList.size())
        {
            ...
            AMap.OnMarkerClickListener l = new AMap.OnMarkerClickListener(){
                ...
                public boolean onMarkerClick(Marker marker)
                {
                    ...
                    intent.putExtra("Owner",c.owner);
                    intent.putExtra("Name",c.getName());
                    intent.putExtra("Lat",c.Lat);
                    intent.putExtra("Lon",c.Lon);
                    intent.putExtra("Intro",c.text);
                    intent.putExtra("Question",c.question);
                    intent.putExtra("Answer",c.answer);
                    intent.putExtra("Direction",c.direction);
                    intent.putExtra("ImgRes",c.imgResource);
